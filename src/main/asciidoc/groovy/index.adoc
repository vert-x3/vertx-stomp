...
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx).handler(StompServerHandler.create(vertx)).listen()
...
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx).handler(StompServerHandler.create(vertx)).listen(1234, "0.0.0.0")
...
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx).handler(StompServerHandler.create(vertx)).listen({ ar ->
  if (ar.failed()) {
...
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx, [
  port:1234,
  host:"0.0.0.0"
...
`link:../../vertx-core/groovy/groovydoc/io/vertx/groovy/core/net/NetServer.html[NetServer]`, so you can configure the underlying `link:../../vertx-core/groovy/groovydoc/io/vertx/groovy/core/net/NetServer.html[NetServer]` from
the `link:../cheatsheet/StompServerOptions.html[StompServerOptions]`. Alternatively you can also pass the
`link:../../vertx-core/groovy/groovydoc/io/vertx/groovy/core/net/NetServer.html[NetServer]` you want to use:

[source,groovy]
----
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx, netServer).handler(StompServerHandler.create(vertx)).listen()
...
Enabling security requires an additional `link:../../vertx-auth-common/groovy/groovydoc/io/vertx/groovy/ext/auth/AuthProvider.html[AuthProvider]` handling the
...
More information about `link:../../vertx-auth-common/groovy/groovydoc/io/vertx/groovy/ext/auth/AuthProvider.html[AuthProvider]` is available
...
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx).handler(StompServerHandler.create(vertx).destinationFactory({ v, name ->
...
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx).handler(StompServerHandler.create(vertx).onAckHandler({ acknowledgement ->
...
import io.vertx.groovy.ext.stomp.StompServerHandler
import io.vertx.groovy.ext.stomp.Stomp
def server = Stomp.createStompServer(vertx).handler(StompServerHandler.create(vertx).closeHandler({ connection ->
...
----
import io.vertx.groovy.ext.stomp.Stomp
def client = Stomp.createStompClient(vertx).connect({ ar ->
  if (ar.succeeded()) {
...
----
import io.vertx.groovy.ext.stomp.Stomp
def client = Stomp.createStompClient(vertx).connect(61613, "0.0.0.0", { ar ->
  if (ar.succeeded()) {
    def connection = ar.result()
...
----
import io.vertx.groovy.ext.stomp.Stomp
def client = Stomp.createStompClient(vertx, [
  host:"localhost",
  port:1234
]).connect({ ar ->
  if (ar.succeeded()) {
...
`link:../cheatsheet/StompClientOptions.html[StompClientOptions]` when creating the `link:groovydoc/io/vertx/groovy/ext/stomp/StompClient.html[StompClient]`. As the
STOMP client relies on a `link:../../vertx-core/groovy/groovydoc/io/vertx/groovy/core/net/NetClient.html[NetClient]`, you can configure the underlying Net Client from
the `link:../cheatsheet/StompClientOptions.html[StompClientOptions]`. Alternatively, you can pass the `link:../../vertx-core/groovy/groovydoc/io/vertx/groovy/core/net/NetClient.html[NetClient]`
...
----
import io.vertx.groovy.ext.stomp.Stomp
def client = Stomp.createStompClient(vertx).connect(netClient, { ar ->
  if (ar.succeeded()) {
    def connection = ar.result()
    connection.errorHandler({ frame ->
      println("ERROR frame received : ${frame}")
